...
const PropertyCard: React.FC<{ property: Property }> = ({ property }) => {
  ...
  return (
    <div className="w-[85%] sm:w-[400px] md:w-full md:flex-1 md:min-w-0 flex-shrink-0 md:flex-shrink snap-center group cursor-pointer flex flex-col gap-4">
      {/* Link around the image for clean routing */}
      <Link
        to={`/property/${property.id}`}
        className="relative overflow-hidden aspect-[4/3] w-full block bg-gray-100 dark:bg-gray-900"
      >
        <img
          alt={property.title}
        ...
        <div
          className={`absolute top-4 left-4 text-[9px] font-bold uppercase tracking-[0.2em] px-3 py-1.5 text-white ${
            isSoldOut ? 'bg-black/85 dark:bg-black/90' : 'bg-[#E06A55]'
          }`}
        >
          {isSoldOut
            ? 'SOLD OUT'
            : property.badge || property.projectStatus || 'READY FOR SALE'}
        </div>
      </Link>
      ...
      <div className="text-left mt-2 flex flex-col gap-2">
        <Link
          to={`/property/${property.id}`}
          className="hover:text-[#E06A55] transition-colors"
        >
...
const PropertiesSection: React.FC = () => {
  ...
  return (
    <section
      className="py-20 md:py-28 bg-[#fafafa] dark:bg-[#0c0c0c]"
      id="properties"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex flex-row justify-between items-end mb-12 border-b border-gray-100 dark:border-gray-900 pb-6">
          <div className="text-left">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.3em] text-red-400 dark:text-red-500 block mb-2">
              THE COLLECTION
            </span>
          ...
          <div className="flex gap-2">
            ...
            >
              <span className="material-icons-outlined text-sm font-bold">
                arrow_back
              </span>
            </button>
            <button
        ...
        {/* Properties: Smooth slider on mobile, balanced equal-width 2-column grid on desktop */}
        <div
          id="property-slider"
          className="flex md:grid md:grid-cols-2 overflow-x-auto md:overflow-visible gap-8 snap-x snap-mandatory scroll-smooth pb-8 no-scrollbar"
